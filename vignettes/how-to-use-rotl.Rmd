...
Looks good, we have IDS for each genus. But if we try to get a subtree from the
Open Tree we get an error:

```{r, eval=FALSE}
tr <- tol_induced_subtree(taxa$ott_id)
```

What's going on? It turns out that _Meleagris_ and _Anas_ are not included in
the synthetic tree (the first because of uncertainty about the rank for that
name and the second because _Anas_ as normally defined is not monophyletic).
