...
test_that("names argument is provided for .tnrs_match_names", {
    skip_on_cran()
    expect_error(.tnrs_match_names(NULL, NULL, TRUE, NULL, FALSE, FALSE),
                 "Must supply")
})

test_that("names argument is character for .tnrs_match_names", {
    skip_on_cran()
    expect_error(.tnrs_match_names(TRUE, NULL, TRUE, NULL, FALSE, FALSE),
                 "character")
})

test_that("names and ids have the same lengths for .tnrs_match_names", {
    skip_on_cran()
    expect_error(.tnrs_match_names("Felis", NULL, TRUE, c("abc", "def"), FALSE, FALSE),
                 "same length")
})

test_that("ids must be character for .tnrs_match_names", {
    skip_on_cran()
    expect_error(.tnrs_match_names("Felis", NULL, TRUE, TRUE, FALSE, FALSE),
                 "character")
})

test_that("do_approximate_matching is logical for .tnrs_match_names", {
    skip_on_cran()
    expect_error(.tnrs_match_names("Felis", NULL, "true", NULL, FALSE, FALSE),
                 "logical")
})

test_that("include_deprecated is logical for .tnrs_match_names", {
    skip_on_cran()
    expect_error(.tnrs_match_names("Felis", NULL, TRUE, NULL, "true", FALSE),
                 "logical")
})

test_that("include_dubious is logical for .tnrs_match_names", {
    skip_on_cran()
    expect_error(.tnrs_match_names("Felis", NULL, TRUE, NULL, FALSE, "true"),
                 "logical")
})
